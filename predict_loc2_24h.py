...
print("場所2（ETTh2: 24時間予測）のデータを読み込み中...")
...
plt.plot(y_test.index[:500], y_pred[:500], label='Predicted (Lag-Corrected 24h)', color='red', linestyle='--', linewidth=1.5)

plt.title(f'Lag-Corrected Prediction (Location: ETTh2, 24h Horizon)\nOverall Test MAE: {mae:.2f}', fontsize=14)
plt.xlabel('Date')
plt.ylabel('Oil Temperature')
plt.legend()
plt.tight_layout()
plt.savefig('spike_detection_ETTh2.png')
print("'spike_detection_ETTh2.png' として保存しました。")