...
print("場所2（ETTm2: 1時間予測）のデータを読み込み中...")
...
plt.plot(y_test[plot_mask].index, y_pred[y_test.index.isin(y_test[plot_mask].index)], label='Predicted (Lag-Corrected 1h)', color='red', linestyle='--', linewidth=1.0)

plt.title(f'Lag-Corrected Prediction (Location: ETTm2)\nPeriod: May 2018 | Overall Test MAE: {mae:.2f}', fontsize=14)
plt.xlabel('Date')
plt.ylabel('Oil Temperature')
plt.legend()
plt.tight_layout()

plt.savefig('spike_detection_ETTm2.png')
print("'spike_detection_ETTm2.png' に保存しました。")